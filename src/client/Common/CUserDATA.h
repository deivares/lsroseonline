...
#else
#define	MAX_BASIC_ABLITY		350		// 250 기본 능력치 최대치를 300으로 수정
...
public :
	...
	// busca rate de EXP do personagem
	int	  GetCur_ExpRate() { return	this->m_iExpRATE;}
...
//	int	  GetTot_DEF_QUALITY()		{	return	this->m_Battle.m_iDefQuality;		}
	...
	void  SetCur_DropRate(__int64 lValue)		{ this->m_iDropRATE = lValue; }
	void  SetCur_ExpRate(int lValue)			{ this->m_iExpRATE = lValue; }
	...
	int					m_iExpRATE = 1;						// Taxa de EXP adicionada por felippe
...
///		m_Battle.m_nWEIGHT -= m_Inventory.GetWEIGHT (wListRealNO);
	...
	virtual void  UpdateModelData() = 0{ *(int*)0 = 11; }		/// 모델 데이터 갱신..
#endif
