	...
	if ( sITEM.GetTYPE() >= ITEM_TYPE_USE && sITEM.GetTYPE() != ITEM_TYPE_RIDE_PART ) {
		...
		switch( nRareType ) {
			case 3 :	// ´ë¸¸ ·¹¾î ¾ÆÀÌÅÛ 
				...
				break;
			case 1 :	// ¹«Á¶°Ç
				//Correção de drop de itens slotados, só permite dropar acessorios, armas e escudos slotados.
				if (sITEM.GetTYPE() >= ITEM_TYPE_JEWEL && sITEM.GetTYPE() <= ITEM_TYPE_SUBWPN) {
