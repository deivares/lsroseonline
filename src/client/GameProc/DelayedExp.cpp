...
void CDelayedExp::GetExp(int iOwnerIndex)
{
	...
	for (; begin != m_ExpData.end(); ++begin)
	{
		...
		if (expData.iOwnerIndex == iOwnerIndex)
		{
			...
			double expTotal = g_pAVATAR->Get_NeedEXP(g_pAVATAR->Get_LEVEL());
			double expPercent = (100 * expData.iAddExp) / expTotal;

			char szMsg[256];
			sprintf(szMsg, STR_GET_EXP, expData.iAddExp, expPercent);
			g_itMGR.AppendChatMsg(szMsg, IT_MGR::CHAT_TYPE_SYSTEM, g_dwYELLOW);
