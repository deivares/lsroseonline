...
const D3DCOLOR	g_dwGray            = D3DCOLOR_ARGB( 255, 127, 127, 127 );
...
{	
	...
	case OBJ_MOB :
		...
		DrawMobName2(x, y, z, pCharOBJ, bTargetObject);
			break;
		}
	case OBJ_NPC :
		{
...
void CNameBox::DrawMobName(float x, float y, float z, CObjCHAR* pCharOBJ, bool bTargeted)
{
	...
	if( bTargeted )
	{
		...
		if( pszMobName )
		{
			...
			size = getFontTextExtent(g_GameDATA.m_hFONT[FONT_NORMAL_OUTLINE], CStr::Printf("%i/%i", pCharOBJ->Get_HP(), pCharOBJ->Get_MaxHP()));
			RECT mrc = { 45 - size.cx / 2 , -3, 55 + size.cx / 2, 27 };
			::drawFont(g_GameDATA.m_hFONT[FONT_NORMAL_OUTLINE], true, &mrc, 0xFFFFFFFF, DT_CENTER, CStr::Printf("%i/%i", pCharOBJ->Get_HP(), pCharOBJ->Get_MaxHP()));
...
void CNameBox::DrawAvatarName( float x, float y, float z, CObjCHAR* pCharOBJ, bool bTargeted )
{
	...
	if (pLevel >= 200 && pLevel < 210) {
		dwColor = g_dwYellowName;
	}

	if (pLevel >= 210 && pLevel < 220) {
		dwColor = g_dwOrangeName;
	}

	if (pLevel >= 220 && pLevel < 230) {
		dwColor = g_dwRedName;
	}

	if (pLevel >= 230 && pLevel < 240) {
		dwColor = g_dwPinkName;
	}

	if (pLevel >= 240 && pLevel < 250) {
		dwColor = g_dwVioletName;
	}

	if (pLevel >= 250 && pLevel < 260) {
		dwColor = g_dwGray;
	}
	...
	if( pName && strlen( pName ) > 3 )
	{
		...
		if (pName[0] == '[' && pName[1] == 'A' && pName[2] == 'D' && pName[3] == 'M' && pName[4] == ']')
		{
			dwColor = 0xFFBB001C;
		}
		...
		if( pName[ 0 ] == '[' && pName[ 1 ] == 'S' && pName[ 2 ] == 'U' && pName[ 3 ] == 'P' && pName[ 4 ] == ']' )
		{
			dwColor = g_dwGREEN;
		}
...
void CNameBox::DrawMyName( float x, float y, float z, CObjCHAR* pCharOBJ, bool bTargeted )
{
	...
	const char* pName = pCharOBJ->Get_NAME();
	int pLevel = pCharOBJ->Get_LEVEL();
	int fontType = FONT_NORMAL_OUTLINE;
	...
	int iHP = pCharOBJ->Get_HP();
	const int iMaxHP = pCharOBJ->Get_MaxHP();
	...
	fGuageDrawX = fGuageDrawX;
	fGuageDrawY = fGuageDrawY + 8;
	...
	RECT rt = { 0, -30, iWidthGuage, iHeightGuage +0 };
	::drawFont(g_GameDATA.m_hFONT[FONT_NORMAL_OUTLINE], true, &rt, dwColor, DT_CENTER | DT_VCENTER, pName);

	int iMP = pCharOBJ->Get_MP();
	const int iMaxMP = pCharOBJ->Get_MaxMP();

	if (iMP < 0)
		iMP = 0;

		if (iMaxHP > 0)
		iWidth = iWidthGuage * iMP / iMaxMP;

	g_DrawImpl.Draw(fGuageDrawX, fGuageDrawY, z, iWidth, IMAGE_RES_UI, CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI00_GUAGE_BLUE_AVATAR"));
...
D3DVECTOR	CNameBox::GetClanMarkDrawPos( CObjCHAR* pChar, float x, float y, float z )
{
	...
	vDrawPos.y = y - NAMEBOX_HEIGHT / 1 - 0;
	vDrawPos.z = z;

	return vDrawPos;
}

void CNameBox::DrawMobName2(float x, float y, float z, CObjCHAR* pCharOBJ, bool bTargeted)
{
	DWORD dwColor = GetTargetMobNameColor(g_pAVATAR->Get_LEVEL(), pCharOBJ->Get_LEVEL());
	...
	if (bTargeted)
	{
		...
		float fGuageDrawX = 20;
		float fGuageDrawY = 170;

		g_DrawImpl.Draw(fGuageDrawX, fGuageDrawY, z, IMAGE_RES_UI, CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI00_GUAGE_BACKGROUND"));
		...
		g_DrawImpl.Draw(fGuageDrawX, fGuageDrawY, z, iWidth, IMAGE_RES_UI, CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI00_GUAGE_RED"));

		fGuageDrawX = fGuageDrawX;
		fGuageDrawY = fGuageDrawY;
