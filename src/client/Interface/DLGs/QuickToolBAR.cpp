...
CQuickBAR::CQuickBAR(int iType, int iQuickBarType)
{
	SetDialogType(iType);
	m_nCurrentPage = 0;
	m_nType = TYPE_HORIZONTAL;

	short iDlgType = DLG_TYPE_QUICKBAR;
	if (iQuickBarType == QUICKBAR_TYPE_EXTENSION)
		iDlgType = DLG_TYPE_QUICKBAR_EXT;
	...
	m_nQuickBarType = iQuickBarType;
	m_nCurrentPage = (m_nQuickBarType == 1 ? MAX_ICONS_PAGES_NORMAL : 0);
}
...
{
	const bool bChatFocus = CTEditBox::s_pFocusEdit != NULL;
	const bool bCtrlActive = GetAsyncKeyState(VK_CONTROL) < 0;

	if (GetQuickBarType() == QUICKBAR_TYPE_NORMAL) {
		if (uiMsg != WM_KEYUP && wParam == VK_F10 && !bCtrlActive) {
			uiMsg = WM_KEYUP;
		}
		if (uiMsg == WM_SYSKEYDOWN) {
			...
			case '4':
			{
				if (GetQuickBarType() == QUICKBAR_TYPE_NORMAL && !bChatFocus)
				{
	...
	else if (GetQuickBarType() == QUICKBAR_TYPE_EXTENSION) {
		if (!bCtrlActive) return 0;
		if (uiMsg == WM_SYSKEYDOWN) {
			switch (wParam) {
			case VK_F1:
			case VK_F2:
...
void CQuickBAR::UpdateCSlotPosition()
{
	...
	if (m_nType == TYPE_HORIZONTAL)
	{
		...
		for (int i = 0; i < HOT_ICONS_PER_PAGE; ++i)
		{
			pt.x = 40 + (m_QuickSlot[0].GetWidth() + 1) * i + (2 * i / 8) * 9;
	...
	else
	{
		...
		{
			pt.y = 41 + (m_QuickSlot[0].GetHeight() + 1) * i + (2 * i / 8) * 9;
			m_QuickSlot[i].SetOffset(pt);
		}
	}
