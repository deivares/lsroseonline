...
UINT CLogin::Process( UINT uiMsg, WPARAM wParam, LPARAM lParam )
{
	...
	if( !IsVision())
		return Ret;
	//Dagnarus
	g_pCApp->SetCaption( CStr::Printf("LS-ROSE Online       [ Login ]"));
	CWinCtrl*  pCtrl	= NULL;
...
void CLogin::Draw()
{
	...
	drawFont( g_GameDATA.m_hFONT[ FONT_NORMAL_OUTLINE ], true, 0, 0, g_dwWHITE, CStr::Printf("LS-ROSE Online, Welcome!"));
#else
