...
//	pFileSystem->CloseFile();
	...
	//Äü¹Ù 
	CQuickBAR* pQuickBAR = new CQuickBAR(DLG_TYPE_QUICKBAR, QUICKBAR_TYPE_NORMAL);
	pQuickBAR->Create("DlgQuickBar");
	AppendDlg( DLG_TYPE_QUICKBAR, pQuickBAR, pQuickBAR->GetControlID() );

	pQuickBAR = new CQuickBAR( DLG_TYPE_QUICKBAR_EXT, QUICKBAR_TYPE_EXTENSION);
	pQuickBAR->Create("DlgQuickBar");
...
CStoreDLG* IT_MGR::GetStoreDLG()
{
	CStoreDLG* pDLG = (CStoreDLG*)FindDlg(DLG_TYPE_STORE);
	return pDLG;
}
...
///°°Àº TYPEÀÇ ¸Þ¼¼Áö ¹Ú½º°¡ ¶°ÀÖ´ÂÁö Ã¼Å©ÇÑ´Ù.
...
//*-------------------------------------------------------------------------------------------/
/// ÇöÀç Æ÷Ä¿½º¸¦ °¡Áö°í ÀÖ´Â ¿¡µðÆ®¹Ú½ºÀÇ Æ÷ÀÎÅÍ(CTEditBox)¸¦ ¸®ÅÏÇÑ´Ù.
//*-------------------------------------------------------------------------------------------/
