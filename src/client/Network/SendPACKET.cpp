...
{	
	...
	//Corrigir problema de ao equipar um colar o nome ficar zuado
	//Isso é uma correçao paliativa, porque a causa raíz nao foi identificada
	char* charname = g_pAVATAR->Get_NAME();
	...
	//Envia o nome antigo do personagem pra atualizar ele e nao ficar zuado.
	this->Send_cli_UPDATE_NAME(charname);
...
{
	//Gambi pra nao zicar o nick do char
	char* charname = g_pAVATAR->Get_NAME();
...
void CSendPACKET::Send_cli_NOEXP(int iExpRate)
{
	m_pSendPacket->m_HEADER.m_wType = GSV_SET_NOEXP;
	m_pSendPacket->m_HEADER.m_nSize = sizeof(gsv_SET_NOEXP);
	m_pSendPacket->m_gsv_SET_NOEXP.m_iExpRATE = iExpRate;
