...
#pragma message("저장된 해상도를 테스트해서 현재 실행가능한지 판단하고 안되면 메세지 박스 띄우고 TriggerDetect를 실행시킨다")
	...
	g_pCApp->CreateWND ("classCLIENT", "LS-ROSE Online       [ Loading ]", Resolution.iWidth, Resolution.iHeight,Resolution.iDepth, hInstance);
	...
	if ( bDeviceInitialized ) {
		...
		///중복실행막은 버젼
		HANDLE  hMUTEX = NULL;
		const char szMUTEX[] = "LS-ROSE Online       [ Avatar ]";
