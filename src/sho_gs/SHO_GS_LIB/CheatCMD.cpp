...
// Cheat para dar bonus DOBRO EXP para alguem durante a sess„o, se deslogar ja era
short classUSER::Cheat_bonusexp(char* pArg1)
{
	classUSER* pUSER = NULL;
	...
	if (pUSER) {
		if (pUSER->GetCur_ExpRate() != 2) {
			pUSER->SetCur_ExpRate(2);
			this->Send_gsv_WHISPER("SERVER", "VOC  EST¡ RECEBENDO O DOBRO DE EXP DE BONUS!");
		}
	}
	return CHEAT_NOLOG;
}
...
short classUSER::Cheat_mm (short nZoneNO, char *pArg2, char *pArg3)
{
	if ( !this->Is_CartGuest() && g_pZoneLIST->IsValidZONE(nZoneNO) ) {
		...
		this->Send_gsv_WHISPER("SERVER", "/mm triggered");
...
// comando /item tipo id quantidade status/socket refine
short classUSER::Cheat_item ( char *pArg1, char *pArg2, char *pArg3, char *pArg4 , char *pArg5)
{
	if ( B_Cheater() ) {
