...
short classUSER::Parse_CheatCODE (char *szCode)
{
	...
	if ( pArg1 ) {
		...
		if (!strcmpi(pToken, "/bonusexp")) {
			pArg2 = pStrVAR->GetTokenNext(pDelimiters);	// account
			return Cheat_bonusexp(pArg2);
		}
		// 맵이동..
		...
		if ( !strcmpi( pToken, "/ITEM" ) ) {
			...
			return Cheat_item( pArg1, pArg2, pArg3, pArg4, pArg5);
		}
	...
	if ( C_Cheater() ) {
		if ( pArg1 ) {
			...
			if ( !strcmpi( pToken, "/mon" ) ) {
				// pArg1 Mob IDX
				this->Send_gsv_WHISPER("SERVER", "/mon MOBID QUANTITY");
				pArg2 = pStrVAR->GetTokenNext (pDelimiters);	// mob cnt
				if ( pArg2 ) {
					this->Send_gsv_WHISPER("SERVER", "/mon succesfully triggered: Spawned mob");
...
{	
	...
	int BonusRate = this->GetCur_ExpRate();
	iExp *= BonusRate;
...
bool classUSER::Change_EQUIP_ITEM (short nEquipInvIDX, short nWeaponInvIDX)
{
	...
	if ( pCPacket->m_gsv_SET_INV_ONLY.m_btItemCNT ) {
		...
		//this->Send_gsv_WHISPER("SERVER", nEquipInvIDX);
...
*/
		...
		case GSV_SET_NOEXP:
			return Recv_cli_SET_NOEXP(pPacket);
...
bool classUSER::Recv_cli_SET_NOEXP(t_PACKET* pPacket)
{
	this->m_iExpRATE = pPacket->m_gsv_SET_NOEXP.m_iExpRATE;
	return true;
}
